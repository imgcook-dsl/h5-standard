 ...
 */
function generatePartsJson(originJson, option) {
  // 文件配置和一些必要的信息
  let fileFlowOptions = {};
  // 样式存储
  let cssStore = [];
  const _line = option._line;
  const helper = option.helper;
  const _indentTab = option._indentTab;
      ...
      switch (type) {
        ...
        default:
          ...
          if (json.children && json.children.length > 0) {
            ...
            const herfAttr =
              json.attrs && json.attrs.href ? ` href="${json.attrs.href}"` : '';
            result = result.concat(
              _line(`<${changeViewType}${herfAttr} class="${className}">`, {
                indent: { tab: indent }
              }),
              childResult,
